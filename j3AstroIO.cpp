...
PhotoPars getPars(const char *file) {
  ...
  try {
    ...
    float xres = -1, yres = -1, fac = -1, xmm = -1, ymm = -1;
...
int write_opencv(const char *ofile, cv::InputArray output, float factor,
                 int depth) {
  int success = 0;
...
int wrtFts(const char *ofile, cv::InputArray outA, int bitpix, int datatype) {
  ...
  for (int c = planes.size() - 1; c >= 0; c--) {
    ...
    T *array = planes[c].isContinuous() ? (T *)planes[c].data
                                        : (T *)planes[c].clone().data;

    fpixel = 1;                      /* first pixel to write      */
    nelements = naxes[0] * naxes[1]; /* number of pixels to write */

    /* write the array of unsigned integers to the FITS file */
    if (fits_write_img(fptr, datatype, fpixel, nelements, array, &status))
      printerror(status);
  }
  printerror(status);
...
int open_fits(const char *file, cv::Mat &image) {
  ...
  long naxes[2], totpix;
  float *array, nulval = 0.; // bscale = 1.0, bzero = 0.0,
  ...
  if (!fits_open_image(&fptr, file, READONLY, &status)) {
    ...
    array = (float *)calloc(
        totpix, bytepix); // TBD will this work also for eg 8bit data? double?
    if (!array) {
    ...
    fits_read_img(fptr, datatype, first, totpix, &nulval, array, &anynul,
                  &status);
...
int open_raw(const char *file, cv::Mat &image) {
  ...
  cv::Mat im = cv::Mat(cv::Size(imag->width, imag->height), CV_16UC3,
                       imag->data, cv::Mat::AUTO_STEP);
  //        .clone();
...
std::string mime(const char *file) {
  struct magic_set *myt =
      magic_open(MAGIC_CONTINUE | MAGIC_ERROR /*|MAGIC_DEBUG*/ |
                 MAGIC_PRESERVE_ATIME | MAGIC_MIME);
  // magic_t myt =
...
#pragma message("USE EMBEDDED MAGIC")
  ...
  if (status != 0) {
    status = magic_load(myt, "/usr/share/file/magic.mgc" /*"./magic.mgc"*/);
    while (status != 0) {
      std::string mgcfile;
      std::cout << "Magic load ERROR -please give the path to magic.mgc file: "
                << std::flush;
      std::cin >> mgcfile;
      status = magic_load(myt, mgcfile.c_str());
      std::cout << status << std::endl;
    }
  }
...
int copyMeta(const char *inFile, const char *outFile) {
  try {
    ...
    try {
      ...
      std::cerr << ": Could not write metadata to (" << outFile << ")\n";
      return 8;
    }
